...
class Heap {
    ...
    public:
        ...
        void insert(T element) {
            ...
            // swap
            heapIndex childIndex = this->tree.size() - 1;
            heapIndex parentIndex = this->getParentPosition(childIndex);
            while (this->tree[childIndex] < this->tree[parentIndex]) {
                std::swap(this->tree[childIndex], this->tree[parentIndex]);
                
                // update index
                childIndex = parentIndex;
                parentIndex = this->getParentPosition(childIndex);
            }
        }
        ...
        void remove(T value) {
            ...
            // look up the value
            heapIndex index = this->find(value, 1);
            if (index == 0) return;
        ...
        heapIndex find(T value, heapIndex i) {
            if (i >= this->tree.size() && this->tree[i] > value) {
                return 0;
            }
            ...
            // if value is less than cur value
            return std::max(find(value, this->getLeftChildPosition(i)), find(value, this->getRightChildPosition(i)));
        }
